...
static struct region alloc_region  ( void const * addr, size_t query ) {
    struct region region = {0, 0, true};
    region.size = region_actual_size(query + offsetof(struct block_header, contents));
    region.addr = map_pages(addr, region.size, MAP_FIXED_NOREPLACE);
    if (region.addr == MAP_FAILED || region_is_invalid(&region)) {
      region.addr = map_pages(addr, region.size, 0);
      region.extends = false;
      if (region_is_invalid(&region) || region.addr == MAP_FAILED) return REGION_INVALID;
    }
...
static bool split_if_too_big( struct block_header* block, size_t query ) {
  if (block_splittable(block, query)) {
        ...
        void *next_address = block->contents + query;
        block_init(next_address, (block_size) {block->capacity.bytes - query}, block->next);
...
static struct block_header* grow_heap( struct block_header* restrict last, size_t query ) {
  query = size_max(query, BLOCK_MIN_CAPACITY);
  block_size sz = {.bytes = query + offsetof(struct block_header, contents)};
  struct region new_region = alloc_region(block_after(last), sz.bytes);
  ...
  struct block_search_result search_result = try_memalloc_existing(query, heap_start);
    switch (search_result.type) {
        ...
        case BSR_REACHED_END_NOT_FOUND:
            ...
            if (search_result.type != BSR_FOUND_GOOD_BLOCK) return NULL;
